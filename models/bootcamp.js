...
const BootcampSchema = new mongoose.Schema({
  ...
  phone: { type: String, maxlength: [20, 'Phone should be under 20 numbers'] },
  email: {
    type: String,
    match: [
      /^(([^<>()[\]\.,;:\s@\"]+(\.[^<>()[\]\.,;:\s@\"]+)*)|(\".+\"))@(([^<>()[\]\.,;:\s@\"]+\.)+[^<>()[\]\.,;:\s@\"]{2,})$/,
  ...
  //Geocoder Point
  //Geocoder must have a type and coord
  location: {
    type: {
      type: String,
      enum: ['Point'],
...
//**In coord array longitude first */
BootcampSchema.pre('save', async function (next) {
  ...
  //Do not save the unformatted address in DB
